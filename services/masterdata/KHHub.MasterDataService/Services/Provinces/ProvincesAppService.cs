...
[Authorize(MasterDataServicePermissions.Provinces.Default)]
public abstract class ProvincesAppServiceBase : ApplicationService
{
    ...
    public virtual async Task<PagedResultDto<ProvinceDto>> GetListAsync(GetProvincesInput input)
    {
        ...
        var items = await _provinceRepository.GetListAsync(input.FilterText, input.Code, input.Name, input.Sorting, input.MaxResultCount, input.SkipCount);
    ...
    [Authorize(MasterDataServicePermissions.Provinces.Delete)]
    public virtual async Task DeleteAsync(Guid id)
    {
        await _provinceRepository.DeleteAsync(id);
    }

    [Authorize(MasterDataServicePermissions.Provinces.Create)]
    public virtual async Task<ProvinceDto> CreateAsync(ProvinceCreateDto input)
    {
        var province = await _provinceManager.CreateAsync(input.Code, input.Name);
        return ObjectMapper.Map<Province, ProvinceDto>(province);
    }

    [Authorize(MasterDataServicePermissions.Provinces.Edit)]
    public virtual async Task<ProvinceDto> UpdateAsync(Guid id, ProvinceUpdateDto input)
    {
        var province = await _provinceManager.UpdateAsync(id, input.Code, input.Name, input.ConcurrencyStamp);
        return ObjectMapper.Map<Province, ProvinceDto>(province);
    }
    ...
    public virtual async Task<IRemoteStreamContent> GetListAsExcelFileAsync(ProvinceExcelDownloadDto input)
    {
        ...
        var items = await _provinceRepository.GetListAsync(input.FilterText, input.Code, input.Name);
    ...
    {
        await _provinceRepository.DeleteAllAsync(input.FilterText, input.Code, input.Name);
    }
    ...
    public virtual async Task<KHHub.MasterDataService.Services.Dtos.Shared.DownloadTokenResultDto> GetDownloadTokenAsync()
    {
        ...
        await _downloadTokenCache.SetAsync(token, new ProvinceDownloadTokenCacheItem { Token = token }, new DistributedCacheEntryOptions { AbsoluteExpirationRelativeToNow = TimeSpan.FromSeconds(30) });
