...
public abstract class EfCorePlaceViewRepositoryBase : EfCoreRepository<MasterDataServiceDbContext, PlaceView, Guid>
{
    ...
    {
        return query.WhereIf(!string.IsNullOrWhiteSpace(filterText), e => e.PlaceView.IpAddress!.Contains(filterText!) || e.PlaceView.Device!.Contains(filterText!) || e.PlaceView.Source!.Contains(filterText!)).WhereIf(userId.HasValue, e => e.PlaceView.UserId == userId).WhereIf(!string.IsNullOrWhiteSpace(ipAddress), e => e.PlaceView.IpAddress.Contains(ipAddress)).WhereIf(!string.IsNullOrWhiteSpace(device), e => e.PlaceView.Device.Contains(device)).WhereIf(viewedAtMin.HasValue, e => e.PlaceView.ViewedAt >= viewedAtMin!.Value).WhereIf(viewedAtMax.HasValue, e => e.PlaceView.ViewedAt <= viewedAtMax!.Value).WhereIf(durationMin.HasValue, e => e.PlaceView.Duration >= durationMin!.Value).WhereIf(durationMax.HasValue, e => e.PlaceView.Duration <= durationMax!.Value).WhereIf(!string.IsNullOrWhiteSpace(source), e => e.PlaceView.Source.Contains(source)).WhereIf(placeId != null && placeId != Guid.Empty, e => e.Place != null && e.Place.Id == placeId);
    }
