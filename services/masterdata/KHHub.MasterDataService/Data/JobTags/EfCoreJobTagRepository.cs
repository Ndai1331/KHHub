...
public abstract class EfCoreJobTagRepositoryBase : EfCoreRepository<MasterDataServiceDbContext, JobTag, Guid>
{
    public EfCoreJobTagRepositoryBase(IDbContextProvider<MasterDataServiceDbContext> dbContextProvider) : base(dbContextProvider)
    {
    }
    ...
    public virtual async Task<List<JobTag>> GetListAsync(string? filterText = null, string? name = null, string? slug = null, string? description = null, int? usageCountMin = null, int? usageCountMax = null, string? sorting = null, int maxResultCount = int.MaxValue, int skipCount = 0, CancellationToken cancellationToken = default)
    {
        var query = ApplyFilter((await GetQueryableAsync()), filterText, name, slug, description, usageCountMin, usageCountMax);
        query = query.OrderBy(string.IsNullOrWhiteSpace(sorting) ? JobTagConsts.GetDefaultSorting(false) : sorting);
    ...
    protected virtual IQueryable<JobTag> ApplyFilter(IQueryable<JobTag> query, string? filterText = null, string? name = null, string? slug = null, string? description = null, int? usageCountMin = null, int? usageCountMax = null)
    {
