...
/// </summary>
public class BaoKhanhHoaArticleSiteHandler : IArticleNewsSiteHandler, ITransientDependency
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (Match m in LdJsonScriptRegex.Matches(html))
        {
            ...
            if (TryFillFromLdJson(raw, patch))
            {
                break;
            }
        }
    ...
    private static bool TryFillFromLdJson(string raw, CrawledArticleDetailPatch patch)
    {
        try
        {
            var node = JsonNode.Parse(raw) as JsonObject ?? TryWrapGraph(raw);
            if (node == null)
            {
                return false;
            }
    ...
    private static JsonObject? TryWrapGraph(string raw)
    {
        try
        {
    ...
    {
        return host.Equals("baokhanhhoa.vn", StringComparison.OrdinalIgnoreCase)
               || host.Equals("www.baokhanhhoa.vn", StringComparison.OrdinalIgnoreCase);
    }

    private static string NormalizeHost(string host)
    {
        return host.StartsWith("www.", StringComparison.OrdinalIgnoreCase) ? host : "www.baokhanhhoa.vn";
    }
