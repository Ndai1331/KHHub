...
helm upgrade --install ${FinalReleaseName} ${ChartName} --namespace ${Namespace} --create-namespace --set global.dotnetEnvironment=${DotnetEnvironment} -f "khhub/values.localdev.yaml" -f "$ChartName/values.${ReleaseName}.yaml"
exit $LASTEXITCODE