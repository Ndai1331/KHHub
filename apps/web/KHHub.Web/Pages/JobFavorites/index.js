$(function () {
    ...
    var jobFavoriteService = window.kHHub.masterDataService.services.jobFavorites.jobFavorites;
    ...
    var createModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'JobFavorites/CreateModal',
        scriptUrl: abp.appPath + 'Pages/JobFavorites/createModal.js',
        modalClass: 'jobFavoriteCreate',
    });

    var editModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'JobFavorites/EditModal',
        scriptUrl: abp.appPath + 'Pages/JobFavorites/editModal.js',
        modalClass: 'jobFavoriteEdit',
    });
    ...
    var dataTableColumns = [
        {
            rowAction: {
                items: [
                    {
                        text: l('Edit'),
                        visible: abp.auth.isGranted('MasterDataService.JobFavorites.Edit'),
                        action: function (data) {
                            editModal.open({
                                id: data.record.jobFavorite.id,
                            });
                        },
                    },
                    {
                        text: l('Delete'),
                        visible: abp.auth.isGranted('MasterDataService.JobFavorites.Delete'),
                        confirmMessage: function () {
                            return l('DeleteConfirmationMessage');
                        },
                        action: function (data) {
                            jobFavoriteService.delete(data.record.jobFavorite.id).then(function () {
    ...
    if (abp.auth.isGranted('MasterDataService.JobFavorites.Delete')) {
    ...
    var dataTable = $('#JobFavoritesTable').DataTable(
        abp.libs.datatables.normalizeConfiguration({
            ...
            responsive: true,
            order: [[2, 'desc']],
            ajax: abp.libs.datatables.createAjax(jobFavoriteService.getList, getFilter),
            ...
            $('#delete-selected-items').click(function () {
                if ($('#select-all-items-btn').data('selected') === true) {
                    abp.message.confirm(l('DeleteAllRecords'), function (confirmed) {
                        ...
                        jobFavoriteService.deleteAll(getFilter()).then(function () {
                            dataTable.ajax.reloadEx();
                            selectOrUnselectAllCheckboxes(false);
                            showOrHideContextMenu();
                        });
                    ...
                    abp.message.confirm(
                        ...
                        function (confirmed) {
                            ...
                            jobFavoriteService.deleteByIds(selectedRecordsIds).then(function () {
                                dataTable.ajax.reloadEx();
                                selectOrUnselectAllCheckboxes(false);
                                showOrHideContextMenu();
                            });
    ...
    $('#NewJobFavoriteButton').click(function (e) {
        e.preventDefault();
        createModal.open();
    });
    ...
    $('#ExportToExcelButton').click(function (e) {
        ...
        jobFavoriteService.getDownloadToken().then(function (result) {
            var input = getFilter();
            var url =
                abp.appPath +
                'api/masterdata/job-favorites/as-excel-file' +
