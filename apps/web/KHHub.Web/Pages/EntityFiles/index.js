$(function () {
    ...
    var createModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'EntityFiles/CreateModal',
        scriptUrl: abp.appPath + 'Pages/EntityFiles/createModal.js',
        modalClass: 'entityFileCreate',
    });

    var editModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'EntityFiles/EditModal',
        scriptUrl: abp.appPath + 'Pages/EntityFiles/editModal.js',
        modalClass: 'entityFileEdit',
    });
    ...
    var getFilter = function () {
        return {
            ...
            collection: $('#CollectionFilter').val(),
            ...
            mediaFileId: $('#MediaFileIdFilter').val(),
        };
    };
    ...
    var dataTableColumns = [
        {
            rowAction: {
                items: [
                    {
                        text: l('Edit'),
                        visible: abp.auth.isGranted('MasterDataService.EntityFiles.Edit'),
                        action: function (data) {
                            editModal.open({
                                id: data.record.entityFile.id,
                            });
                        },
                    },
                    {
                        text: l('Delete'),
                        visible: abp.auth.isGranted('MasterDataService.EntityFiles.Delete'),
                        confirmMessage: function () {
                            return l('DeleteConfirmationMessage');
                        },
                        action: function (data) {
                            entityFileService.delete(data.record.entityFile.id).then(function () {
        ...
        { data: 'entityFile.entityType' },
        { data: 'entityFile.entityId' },
        { data: 'entityFile.collection' },
        { data: 'entityFile.sortOrder' },
        {
            data: 'entityFile.isPrimary',
    ...
    if (abp.auth.isGranted('MasterDataService.EntityFiles.Delete')) {
    ...
    var dataTable = $('#EntityFilesTable').DataTable(
        abp.libs.datatables.normalizeConfiguration({
            ...
            responsive: true,
            order: [[2, 'desc']],
            ajax: abp.libs.datatables.createAjax(entityFileService.getList, getFilter),
            ...
            $('#delete-selected-items').click(function () {
                if ($('#select-all-items-btn').data('selected') === true) {
                    abp.message.confirm(l('DeleteAllRecords'), function (confirmed) {
                        ...
                        entityFileService.deleteAll(getFilter()).then(function () {
                            dataTable.ajax.reloadEx();
                            selectOrUnselectAllCheckboxes(false);
                            showOrHideContextMenu();
                        });
                    ...
                    abp.message.confirm(
                        ...
                        function (confirmed) {
                            ...
                            entityFileService.deleteByIds(selectedRecordsIds).then(function () {
                                dataTable.ajax.reloadEx();
                                selectOrUnselectAllCheckboxes(false);
                                showOrHideContextMenu();
                            });
    ...
    $('#NewEntityFileButton').click(function (e) {
        e.preventDefault();
        createModal.open();
    });
