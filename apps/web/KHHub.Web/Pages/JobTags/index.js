$(function () {
    ...
    var jobTagService = window.kHHub.masterDataService.services.jobTags.jobTags;

    var createModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'JobTags/CreateModal',
        scriptUrl: abp.appPath + 'Pages/JobTags/createModal.js',
        modalClass: 'jobTagCreate',
    });

    var editModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'JobTags/EditModal',
        scriptUrl: abp.appPath + 'Pages/JobTags/editModal.js',
        modalClass: 'jobTagEdit',
    });
    ...
    var getFilter = function () {
        return {
            ...
            usageCountMin: $('#UsageCountFilterMin').val(),
            usageCountMax: $('#UsageCountFilterMax').val(),
        };
    };

    var canEdit = abp.auth.isGranted('MasterDataService.JobTags.Edit');
    var canDeleteRow = abp.auth.isGranted('MasterDataService.JobTags.Delete');
    ...
    if (abp.auth.isGranted('MasterDataService.JobTags.Delete')) {
    ...
    var dataTable = $('#JobTagsTable').DataTable(
        abp.libs.datatables.normalizeConfiguration({
            ...
            responsive: true,
            order: [[2, 'desc']],
            ajax: abp.libs.datatables.createAjax(jobTagService.getList, getFilter),
            columnDefs: dataTableColumns,
        })
    );

    $('#JobTagsTable').on('click', '.action-edit', function (e) {
        e.preventDefault();
        e.stopPropagation();
        editModal.open({ id: $(this).data('id') });
    });

    $('#JobTagsTable').on('click', '.action-delete', function (e) {
        e.preventDefault();
        e.stopPropagation();
        var id = $(this).data('id');
        ...
        abp.message.confirm(l('DeleteConfirmationMessage')).then(function (confirmed) {
            ...
            jobTagService.delete(id).then(function () {
            ...
            $('#delete-selected-items').click(function () {
                if ($('#select-all-items-btn').data('selected') === true) {
                    abp.message.confirm(l('DeleteAllRecords'), function (confirmed) {
                        ...
                        jobTagService.deleteAll(getFilter()).then(function () {
                            dataTable.ajax.reloadEx();
                            selectOrUnselectAllCheckboxes(false);
                            showOrHideContextMenu();
                        });
                    ...
                    abp.message.confirm(
                        ...
                        function (confirmed) {
                            ...
                            jobTagService.deleteByIds(selectedRecordsIds).then(function () {
                                dataTable.ajax.reloadEx();
                                selectOrUnselectAllCheckboxes(false);
                                showOrHideContextMenu();
                            });
    ...
    $('#NewJobTagButton').click(function (e) {
        e.preventDefault();
        createModal.open();
    });
    ...
    $('#ExportToExcelButton').click(function (e) {
        ...
        jobTagService.getDownloadToken().then(function (result) {
            var input = getFilter();
            var url =
                abp.appPath +
                'api/masterdata/job-tags/as-excel-file' +
                abp.utils.buildQueryString([
                    ...
                    { name: 'usageCountMin', value: input.usageCountMin },
                    { name: 'usageCountMax', value: input.usageCountMax },
                ]);
