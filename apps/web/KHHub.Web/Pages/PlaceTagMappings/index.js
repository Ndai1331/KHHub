$(function () {
    ...
    var dataTableColumns = [
        {
            rowAction: {
                items: [
                    {
                        text: l('Edit'),
                        visible: abp.auth.isGranted('MasterDataService.PlaceTagMappings.Edit'),
                        action: function (data) {
                            editModal.open({
                                id: data.record.placeTagMapping.id,
                            });
                        },
                    },
                    {
                        text: l('Delete'),
                        visible: abp.auth.isGranted('MasterDataService.PlaceTagMappings.Delete'),
                        confirmMessage: function () {
                            return l('DeleteConfirmationMessage');
                        },
                        action: function (data) {
                            placeTagMappingService
                                .delete(data.record.placeTagMapping.id)
                                .then(function () {
    ...
    if (abp.auth.isGranted('MasterDataService.PlaceTagMappings.Delete')) {
            ...
            $('#delete-selected-items').click(function () {
                if ($('#select-all-items-btn').data('selected') === true) {
                    abp.message.confirm(l('DeleteAllRecords'), function (confirmed) {
                        ...
                        placeTagMappingService.deleteAll(getFilter()).then(function () {
                            dataTable.ajax.reloadEx();
                            selectOrUnselectAllCheckboxes(false);
                            showOrHideContextMenu();
                        });
